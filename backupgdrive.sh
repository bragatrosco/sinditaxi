...
#        split -b 2000M ${WORKING_DIR}/atom/$(basename ${folder}).tar.xz "root_backup_"
#        cd - > /dev/null
#done
##################################
...
curday=$(date +%F)
gdrive mkdir backup_${SERVER}_${curday} 2> /dev/null

# Backup now!!!
gdrive upload ${WORKING_DIR} -p backup_${SERVER}_${curday} > /dev/null
